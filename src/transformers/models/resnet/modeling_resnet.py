...
class ResNetForImageClassification(ResNetPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        pooled_output = outputs.pooler_output if isinstance(outputs, BaseModelOutputWithPoolingAndNoAttention) else outputs[1]
...
class ResNetBackbone(BackboneMixin, ResNetPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Run through encoder stages with hidden state collection
        hidden_states_list = [embedding_output]
        hidden_state = embedding_output
        for stage in self.encoder.stages:
            hidden_state = stage(hidden_state)
            hidden_states_list.append(hidden_state)

        # Extract feature maps based on out_features
        feature_maps = ()
        for idx, stage in enumerate(self.stage_names):
            if stage in self.out_features:
                feature_maps += (hidden_states_list[idx],)

        return BackboneOutput(
            feature_maps=feature_maps,
            hidden_states=tuple(hidden_states_list) if kwargs.get('output_hidden_states', self.config.output_hidden_states) else None,
            attentions=None,
        )
